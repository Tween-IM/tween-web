...
**Single User Resolution Response Fields:**

| Field | Required | Type | Description |
|-------|----------|-------|-------------|
| `user_id` | Yes | string | Matrix User ID |
| `wallet_id` | Yes | string | Wallet identifier |
| `wallet_status` | Yes | enum | Wallet status (always `active`) |
| `user_status` | Yes | enum | User account status (`active`, `pending`, `suspended`) |
| `payment_enabled` | Yes | boolean | Whether user can send payments |
| `display_name` | No | string | User display name |
| `avatar_url` | No | string | Matrix avatar URL |
| `created_at` | No | string | ISO 8601 timestamp |
| `activation_required` | No | boolean | Whether activation required (`user_status: pending`) |
| `activation_url` | No | string | Activation URL |
| `pending_balance` | No | object | Balance amount pending activation |

**Batch Resolution Response Fields:**
...
Wallet Service implementations MUST provide the following capabilities:
...
- If the account does not exist, the Wallet Service MUST create a new account
...
- Upon successful activation, the Wallet Service MUST set `user_status: active` and `payment_enabled: true`
...
2. Resolve recipient Matrix ID to wallet_id:
   ...
   - If no account found, call `EnsureUserAccount("@bob:tween.example")` to create account with user_status:pending
...
When the TMCP Server calls the account creation method for a Matrix user without an account:
...
The TMCP Server SHOULD only invoke account creation when there is explicit intent to transfer funds to the recipient. This prevents creation of unwanted wallet accounts for privacy reasons and reduces potential for account enumeration attacks.

**Activation Notification:**

When a payment is successfully sent to a user with `user_status: pending`:

- The TMCP Server MUST send a Matrix event of type `m.tween.account.activate` to the recipient in the relevant room
...
#### 6.3.10 Account Activation Notification

When a user attempts to send money to a user whose account is pending activation:
...
```json
{
  ...
  "content": {
    "msgtype": "m.tween.activation_invite",
    "body": "Alice sent you money! Activate your account to access it.",
...
The Wallet Service MUST implement the following capabilities for withdrawal operations:

**Withdrawal Initiation:**
- The Wallet Service MUST support initiating withdrawals with user_id, destination, and amount
- The Wallet Service MUST return a unique withdrawal_id upon initiation
- The Wallet Service MUST validate user has sufficient balance before initiation

**Withdrawal Approval:**
- The Wallet Service MUST support withdrawal approval via approval_data
- The approval_data MUST include required authentication and verification information
- The Wallet Service MUST return the updated withdrawal status after approval

**Withdrawal Status Query:**
- The Wallet Service MUST support querying withdrawal details by withdrawal_id
- The response MUST include: current status, amount, destination, and timestamps
...
| `USER_SUSPENDED` | 403 | Recipient user account is suspended (payment initiation only) | No |
